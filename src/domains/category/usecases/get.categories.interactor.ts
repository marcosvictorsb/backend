...
export class GetCategoriesInteractor {
  constructor(private readonly gateway: IGetCategoriesGateway, private presenter: IPresenter) {}

  async execute(input: InputGetCategories): Promise<HttpResponse> {   
    try {
      const { id_user } = input;
      this.gateway.loggerInfo('Iniciado a requisição para buscar as cateroria', { data: JSON.stringify(input) });
