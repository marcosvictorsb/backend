import { IncomeStatus, IUpdateIncomeGateway, UpdateIncomeData } from '../interfaces';
...
export class UpdateIncomeInteractor {
  ...
  async execute(input: InputUpdateIncome): Promise<HttpResponse> {   
    try {
      ...
      if (!currentIncome) {
        this.gateway.loggerInfo('Receita não encontrada', { requestTxt: JSON.stringify(input) });
        return this.presenter.notFound('Receita não encontrada');
      }

      const isChangingBank = id_bank && currentIncome.id_bank !== id_bank;
      const oldBank = await this.gateway.findBank({ id: currentIncome.id_bank });
      if (isChangingBank && !oldBank) {
          this.gateway.loggerInfo('Banco antigo não encontrado', { id_bank: currentIncome.id_bank });
      ...
      if (status === IncomeStatus.RECEIVED) {
        ...
        if (!newBank) {
          this.gateway.loggerInfo('Novo banco não encontrado', { id_bank: id_bank });
          return this.presenter.notFound('Banco não encontrado');
        }

        if (isChangingBank) {
          const oldBankNewAmount = oldBank!.amount - currentIncome.amount;
          await this.gateway.updateBank({ id: oldBank!.id, amount: oldBankNewAmount });
          
          const newBankNewAmount = newBank.amount + amount;
          await this.gateway.updateBank({ id: newBank.id, amount: newBankNewAmount });
        } else {
          const amountDifference = amount - currentIncome.amount;
          const newBankAmount = newBank.amount + amountDifference;
          await this.gateway.updateBank({ id: newBank.id, amount: newBankAmount });
        }
      } else if (currentIncome.status === IncomeStatus.RECEIVED && status !== IncomeStatus.RECEIVED) {
      ...
      const updateCriteria: UpdateIncomeData = { id, amount, description, id_user, status, id_bank };
