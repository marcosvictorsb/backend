...
export class UpdateExpenseInteractor {
  constructor(private readonly gateway: IUpdateExpenseGateway, private presenter: IPresenter) {}

  async execute(input: InputUpdateExpense): Promise<HttpResponse> {   

    try {
      this.gateway.loggerInfo('Atualizando despesa', { requestTxt: JSON.stringify(input) });
